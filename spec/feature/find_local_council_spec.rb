...
RSpec.describe "FindLocalCouncil", type: :feature do
  ...
  context "when visiting the start page" do
    before do
      allow(Frontend).to receive(:govuk_website_root).and_return("https://www.test.gov.uk")
    ...
    it "adds google analytics tags for postcodeSearchStarted" do
      ...
      expect(track_category).to eq("postcodeSearch:find_local_council")
    ...
    context "for successful postcode lookup" do
      context "for unitary local authority" do
        ...
        it "adds google analytics for postcodeResultsShown" do
          ...
          expect(track_label).to eq("1 Result")
        end

        it "adds google analytics for exit link tracking" do
          track_action = find_link("Go to Westminster website")["data-track-action"]
          expect(track_action).to eq("unitaryLinkClicked")
        end
      ...
      context "for district local authority" do
        ...
        it "adds google analytics for postcodeResultsShown" do
          ...
          expect(track_category).to eq("postcodeSearch:find_local_council")
          expect(track_action).to eq("postcodeResultShown")
          expect(track_label).to eq("2 Results")
        end
        ...
        it "adds google analytics for exit link tracking" do
          ...
          expect(district_track_action).to eq("districtLinkClicked")
          expect(county_track_action).to eq("countyLinkClicked")
        end
    ...
    context "for unsuccessful postcode lookup" do
      context "with invalid postcode" do
        ...
        it "populates google analytics tags" do
          ...
          expect(track_label).to eq("This isn't a valid postcode.")
        end
      ...
      context "when no local council is found" do
        ...
        it "populates google analytics tags" do
          ...
          expect(track_label).to eq("We couldn't find a council for this postcode.")
        end
      end
    end
  end
