RSpec.describe "CallForEvidence" do
  # it_behaves_like "it has meta tags", "call_for_evidence", "/government/calls-for-evidence/youth-vaping-call-for-evidence", "open_call_for_evidence"
  # it_behaves_like "it has meta tags for images", "call_for_evidence", "/government/calls-for-evidence/youth-vaping-call-for-evidence", "open_call_for_evidence"
  ...
  context "when visiting an open call for evidence page" do
    ...
    it "displays the metadata with logo" do
      within(".gem-c-metadata") do
        ...
        expect(page).to have_content("Last updated 4 October 2023")
    ...
    it "displays the single page notification button at the top and bottom of page" do
      ...
      notification_button.each do
        expect(page).to have_content("Get emails about this page")
      end
    end

    it "displays the history notice if there is government information available" do
      within(".gem-c-notice__title") do
        expect(page).to have_content("This was published under the #{@content_store_response['details']['government']['title']}")
      end
    end
    ...
    context "when it displays the summary box" do
      ...
      it "displays the description" do
        within(".app-c-banner") do
          expect(page).to have_css(".app-c-banner__desc", text: @content_store_response["details"]["description"])
        end
      end
    ...
    it "links to external url call for evidence page" do
      expect(page).to have_link("another website", href: @content_store_response["details"]["held_on_another_website_url"])
    end
    ...
    context "when it displays the ways to respond" do
      before do
        ...
        @ways_to_respond = content_store_response["details"]["ways_to_respond"]
  ...
  context "when visiting a call for evidence outcome page" do
    ...
    context "when attachments are available" do
      ...
      it "displays accessible format option when accessible is false and email is supplied" do
        expected_attachment = @content_store_response["details"]["attachments"][0]
        attachments = page.find_all(".gem-c-attachment")
      ...
      it "does not display ccessible format option when accessible is false and email is not supplied" do
        overrides = {
          "details" => {
            "attachments" => [
              {
  ...
  context "when visiting an unopened call for evidence page" do
    before do
      # @content_store_response = GovukSchemas::Example.find("call_for_evidence", example_name: "unopened_call_for_evidence")
