...
RSpec.describe SessionsController, type: :controller do
  ...
  context "GET /account" do
    ...
    it "redirects the user to the GOV.UK Account service domain" do
      ...
      expect(@response.headers["Location"]).to eq("https://home.account.gov.uk")
    end

    it "preserves the _ga tracking parameter if provided" do
      get(:create, params: { _ga: "ga123" })

      expect(@response.headers["Location"]).to eq("https://home.account.gov.uk?_ga=ga123")
    end
  ...
  context "GET /sign-in/callback" do
    ...
    it "redirects to the root path if the :code param is not present" do
      ...
      assert_response(:redirect)
      expect(@root_path).to eq(@response.redirect_url)
    end
  ...
  context "GET /sign-out" do
    context "when the user is logged in" do
      ...
      it "sets the 'GOVUK-Account-End-Session' header to 1" do
        ...
        expect(@response.headers["GOVUK-Account-End-Session"].present?).to be true
      end
  ...
  def stub_account_api
    stub_account_api_validates_auth_response(govuk_account_session: @govuk_account_session, redirect_path: @redirect_path, cookie_consent: @cookie_consent, feedback_consent: @feedback_consent)
  end
end
