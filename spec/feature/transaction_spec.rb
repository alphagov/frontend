RSpec.describe "Transaction", type: :feature do
  ...
  context "a transaction with all the optional things" do
    before do
      @payload = { analytics_identifier: nil, base_path: "/carrots", content_id: "d6d6caaf-77db-47e1-8206-30cd4f3d0e3f", document_type: "transaction", first_published_at: "2016-02-29T09:24:10.000+00:00", locale: "en", phase: "beta", public_updated_at: "2012-10-22T12:49:50.000+00:00", publishing_app: "publisher", rendering_app: "frontend", schema_name: "transaction", title: "Carrots", updated_at: "2012-10-22T12:30:33.483Z", withdrawn_notice: {}, links: {}, description: "Descriptive carrots text.", details: { introductory_paragraph: "This is the introduction to carrots\n          <h2>Next bit</h2>If you'd like some carrots, you need to prove that you're not a rabbit", transaction_start_link: "http://carrots.example.com", will_continue_on: "Carrotworld", start_button_text: "Eat Carrots Now", what_you_need_to_know: "Includes carrots", downtime_message: "CarrotServe will be offline next week." }, external_related_links: [] }
    ...
    it "renders the main information" do
      ...
      within("#content") do
        within(".gem-c-title") { assert_has_component_title("Carrots") }
        within(".article-container") do
          within("section.intro") do
            expect(page).to have_selector(".get-started-intro", text: "This is the introduction to carrots")
            assert_has_button_as_link("Eat Carrots Now", href: "http://carrots.example.com", start: true, rel: "external")
    ...
    it "contains GovernmentService schema.org information" do
      ...
      expected_service = { "@context" => "http://schema.org", "@type" => "GovernmentService", "name" => "Carrots", "description" => "Descriptive carrots text.", "url" => "http://www.dev.gov.uk/carrots", "provider" => [{ "@type" => "GovernmentOrganization", "name" => "Department for Carrots", "url" => "https://www.gov.uk/department-for-carrots" }] }
  ...
  context "jobsearch page" do
    it "renders ok" do
      ...
      expect(page.status_code).to eq(200)
      assert_has_button_as_link("Start now", href: "https://jobsearch.direct.gov.uk/JobSearch/PowerSearch.aspx")
    end
  end
  ...
  context "start page which should have cross domain analytics" do
    it "includes cross domain analytics javascript" do
      ...
      expect(page.status_code).to eq(200)
      assert_has_button_as_link("Start now", rel: "external", href: "http://cti.voa.gov.uk/cti/inits.asp", start: true, data_attributes: { "module" => "cross-domain-tracking", "tracking-code" => "UA-12345-6", "tracking-name" => "transactionTracker" })
    end
  end
  ...
  context "start page format which shouldn't have cross domain analytics" do
    it "does not include cross domain analytics javascript" do
      ...
      expect(page.status_code).to eq(200)
      assert_has_button_as_link("Start now", rel: "external", start: true, href: "https://jobsearch.direct.gov.uk/JobSearch/PowerSearch.aspx")
    end
  end

  context "locale is 'cy'" do
    before do
      @payload = { base_path: "/cymraeg", content_id: "d6d6caaf-77db-47e1-8206-30cd4f3d0e3f", document_type: "transaction", locale: "cy", publishing_app: "publisher", rendering_app: "frontend", schema_name: "transaction", title: "Cymraeg", description: "Cynnwys Cymraeg", details: { transaction_start_link: "http://cymraeg.example.com", start_button_text: "Start now" } }
    ...
    it "renders start button text 'Dechrau nawr'" do
      ...
      within(".article-container") do
        within("section.intro") do
          assert_has_button_as_link("Dechrau nawr", rel: "external", start: true, href: "http://cymraeg.example.com")
        end
      end
    end
  end
  ...
  context "a transaction has variants" do
    it "renders correct content including robots meta tag" do
      ...
      expect(page.status_code).to eq(200)
      assert_has_button_as_link("Start now", href: "http://cti-staging.voa.gov.uk/cti/inits.asp")
