RSpec.describe GovukPersonalisationHelper do
  ...
  describe "#email_subscription_success_banner_heading" do
    it "displays the subscription success banner when the 'email-subscription-success' flash is present" do
      account_flash = request.headers["GOVUK-Account-Session"] = GovukPersonalisation::Flash.encode_session("session-id", %w[email-subscription-success])
      expect(email_subscription_success_banner_heading(account_flash)).to eq("You’ve subscribed to emails about this page")
    end

    it "displays the unsubscription success banner when the 'email-unsubscribe-success' flash is present" do
      account_flash = request.headers["GOVUK-Account-Session"] = GovukPersonalisation::Flash.encode_session("session-id", %w[email-unsubscribe-success])
      expect(email_subscription_success_banner_heading(account_flash)).to eq("You’ve unsubscribed from emails about this page")
    end
    ...
    it "displays the subscription already subscribed banner when the 'email-subscription-already-subscribed' flash is present" do
      account_flash = request.headers["GOVUK-Account-Session"] = GovukPersonalisation::Flash.encode_session("session-id", %w[email-subscription-already-subscribed])
      expect(email_subscription_success_banner_heading(account_flash)).to eq("You’re already getting emails about this page")
    end

    it "does not display a banner when the flash is not present" do
      account_flash = request.headers["GOVUK-Account-Session"] = GovukPersonalisation::Flash.encode_session("session-id", [])
