RSpec.describe RecruitmentBannerHelper, type: :view do
  ...
  it "recruitment_banner returns banners that include the current url" do
  ...
  it "recruitment_banners yaml structure is valid" do
    ...
    if @recruitment_banners_data["banners"].present?
      recruitment_banners.each do |banner|
        ...
        expect(banner["suggestion_text"].blank?).to be true
        expect(banner.key?("suggestion_link_text")).to be true
        expect(banner["suggestion_link_text"].blank?).to be true
        expect(banner.key?("survey_url")).to be true
        expect(banner["survey_url"].blank?).to be true
        expect(banner.key?("page_paths")).to be true
        expect(banner["page_paths"].blank?).to be true
      end
    end
  end
end
