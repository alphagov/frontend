class CorporateInformationPagePresenter < ContentItemModelPresenter
  ...
  def corporate_information
    corporate_information_groups.map do |group|
      {
        heading: ApplicationController.new.view_context.tag.h3(
          group["name"],
  ...
  def corporate_information_heading_tag
    ApplicationController.new.view_context.tag.h2(
...
private
  ...
  def further_information_link(type)
    ...
    ApplicationController.new.view_context.link_to(link["title"], link["base_path"]) if link
  end
  ...
  def normalised_group_item_link(group_item)
    if group_item.is_a?(String)
      ...
      ApplicationController.new.view_context.link_to(group_item_link["title"], group_item_link["base_path"]) if group_item_link
    else
      ApplicationController.new.view_context.link_to(group_item["title"], group_item["path"] || group_item["url"])
    end
  end
