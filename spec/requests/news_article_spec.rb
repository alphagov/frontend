...
RSpec.describe "News Article" do
  ...
  describe "GET show" do
    ...
    context "when loaded from GraphQL" do
      let(:api_response) { graphql_has_example_item("news_article") }

      before do
        base_path = api_response.fetch("base_path")
      ...
      context "when the content item has (upto 5 levels of) taxonomy" do
        let(:api_response) { graphql_has_example_item("news_article_with_taxons") }
