RSpec.describe SimpleSmartAnswersController, type: :controller do
  include GovukAbTesting::RspecHelpers

  def simple_smart_answer_content_item
    { base_path: "/the-bridge-of-death", document_type: "simple_smart_answer", schema_name: "simple_smart_answer", title: "The bridge of death", description: "Cheery description about bridge of death", external_related_links: [] }
  end

  def simple_smart_answer_content_item_with_start_button_text
    { base_path: "/the-squirrel-of-doom", document_type: "simple_smart_answer", schema_name: "simple_smart_answer", title: "The squirrel of doom", description: "Noooo, not the squirrel of doom!", details: { start_button_text: "Start now" }, external_related_links: [] }
  end
  ...
  context "GET show" do
    ...
    context "for live content" do
      it "sets the cache expiry headers" do
        get(:show, params: { slug: "the-bridge-of-death" })
  ...
  context "GET 'flow'" do
    context "for a simple_smart_answer slug" do
      before do
        @node_details = [{ "kind" => "question", "slug" => "question-1", "title" => "Question 1", "body" => "<p>This is question 1</p>", "options" => [{ "label" => "Option 1", "slug" => "option-1", "next_node" => "question-2" }, { "label" => "Option 2", "slug" => "option-2", "next_node" => "outcome-1" }, { "label" => "Option 3", "slug" => "option-3", "next_node" => "outcome-2" }] }, { "kind" => "question", "slug" => "question-2", "title" => "Question 2", "body" => "<p>This is question 2</p>", "options" => [{ "label" => "Option 1", "slug" => "option-1", "next_node" => "outcome-1" }, { "label" => "Option 2", "slug" => "option-2", "next_node" => "outcome-2" }] }, { "kind" => "outcome", "slug" => "outcome-1", "title" => "Outcome 1", "body" => "<p>This is outcome 1</p>" }, { "kind" => "outcome", "slug" => "outcome-2", "title" => "Outcome 2", "body" => "<p>This is outcome 2</p>" }]
        payload = simple_smart_answer_content_item.merge(details: { start_button_text: "Start here", body: "Hello", nodes: @node_details })
      ...
      it "calculates the flow state with no responses" do
        ...
        allow(SimpleSmartAnswers::Flow).to receive(:new).with(@node_details).and_return(flow)
      ...
      it "calculates the flow state for the given responses" do
        ...
        allow(SimpleSmartAnswers::Flow).to receive(:new).with(@node_details).and_return(flow)
        allow(flow).to receive(:state_for_responses).with(%w[option-1 option-2]).and_return(state)
      ...
      context "with form submission params" do
        ...
        it "redirects to the canonical path for the resulting state" do
          ...
          assert_redirected_to(action: :flow, responses: "option-1/option-2", slug: "the-bridge-of-death")
        end
        ...
        it "passes on the 'token' param when in fact check" do
          ...
          assert_redirected_to(action: :flow, slug: "the-bridge-of-death", responses: "option-1/option-2", params: { token: "123" })
        end
      end
    end
  end
