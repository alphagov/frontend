RSpec.describe CsvPreviewService, type: :model do
  before do
    @csv = "header 1,header 2,header 3\ncontent 1,content 2,content 3\n"
    @long_csv = @csv.dup
    (CsvPreviewService::MAXIMUM_ROWS + 10).times do
      @long_csv = "#{@long_csv}other content 1,other content 2,other content 3\n"
    end
    @crlf_csv = @csv.gsub(/\n/, "\r\n")
    @csv_with_many_columns = "column"
    (CsvPreviewService::MAXIMUM_COLUMNS + 10).times do
      @csv_with_many_columns = "#{@csv_with_many_columns},column"
    end
    (@csv_with_many_columns << "\n")
    @parsed_csv = [[{ text: "header 1" }, { text: "header 2" }, { text: "header 3" }], [{ text: "content 1" }, { text: "content 2" }, { text: "content 3" }]]
  end
  ...
  describe "#csv_rows" do
    ...
    context "with long CSV" do
      ...
      it "returns only the header row and less or equal to the maximum number of normal rows" do
        expect(subject.first.length).to be <= (CsvPreviewService::MAXIMUM_ROWS + 1)
      end
    ...
    context "with CSV containing many columns" do
      ...
      it "returns only less than or equal the maximum number of columns" do
        expect(subject.first.first.length).to be <= CsvPreviewService::MAXIMUM_COLUMNS
      end
    ...
    context "with CSV in UTF-8" do
      subject do
        CsvPreviewService.new("za\u017C\u00F3\u0142\u0107 g\u0119\u015Bl\u0105 ja\u017A\u0144\n").csv_rows
      end
    ...
    context "with CSV in windows-1252" do
      subject do
        CsvPreviewService.new("\xFE\xE6r he feoll his tw\xE6gen gebro\xF0ra\n").csv_rows
      end
    ...
    context "with CSV with bytes that cannot be converted to UTF-8" do
      ...
      it "raises FileEncodingError" do
        expect { subject.csv_rows }.to raise_error(CsvPreviewService::FileEncodingError)
      end
    end
  end
