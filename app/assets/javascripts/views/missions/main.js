const apiKey = 'VLriVg86vFJIQ6wFczR6dAPJvZyqgK05';
...
// const icbNameTemplate = "NHS {name} Integrated Care Board";
// const nhsNameTemplate = "{name} NHS Region";
...
const lkupPromise = fetch('../lookup.json').then((response) => response.json());
const icbPromise = fetch('../data/icb.geojson').then((response) => response.json());
const cdcPromise = fetch('../data/cdc.csv').then((response) => response.text());
const hubPromise = fetch('../data/hub.csv').then((response) => response.text());

Promise.all([ lkupPromise, icbPromise, cdcPromise, hubPromise ]).then((values) => {
    const lkupData = values[0];
    const icbData = values[1];
    const cdcData = values[2];
    const hubData = values[3];

    icbLookup = lkupData.icb;
    nhsLookup = lkupData.nhs;

    //Add the DHSC Community Diagnostic Centre (CDC) locations.
    map.createPane('cdc');
    map.getPane('cdc').style.zIndex = 650;

    const cdcCustomOverlay = L.geoJson(null, {
    ...
    const cdcOverlay = omnivore.csv.parse(cdcData, null, cdcCustomOverlay);

    //Add the DHSC surgical hub locations.
    map.createPane('hub');
    map.getPane('hub').style.zIndex = 651;

    const hubCustomOverlay = L.geoJson(null, {
        onEachFeature: bindPopup,
        pointToLayer: function (feature, latlng) {
            return L.circleMarker(latlng);
        },
    ...
    const hubOverlay = omnivore.csv.parse(hubData, null, hubCustomOverlay);
    ...
    icbData.features.forEach((element) => {
    ...
    for (const [ key, value ] of Object.entries(nhsLookup)) {
        icbOverlays[key] = L.geoJson(icbData, {
            onEachFeature: bindPopup,
...
function bindPopup(feature, layer) {
    ...
    // properties = (({ column_name, ...o }) => o)(properties);
