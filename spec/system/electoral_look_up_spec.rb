RSpec.describe "ElectoralLookUp" do
    ...
    context "when a valid postcode is entered which matches multiple ERO addresses" do
      it "displays an address picker" do
        ...
        with_multiple_addresses["addresses"] = [
          { "address" => "1 BUCKINGHAM PALACE", "postcode" => postcode, "slug" => "1234", "url" => "/foo" },
          { "address" => "2 BUCKINGHAM PALACE", "postcode" => postcode, "slug" => "5678", "url" => "/bar" },
        ]
    ...
    context "when the API returns 500" do
      ...
      it "displays election service not available message" do
        with_electoral_api_url do
          ...
          expect(page).to have_text(
            "This service is currently experiencing technical difficulties. Try again later if it does not work.",
            count: 1,
          )
        end
      end
