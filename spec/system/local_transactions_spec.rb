...
RSpec.describe "LocalTransactions", type: :system do
  ...
  context "given a local transaction with an interaction present" do
    ...
    context "when visiting the local transaction with a valid postcode" do
      ...
      it "shows a get started button which links to the interaction" do
        expect(page).to have_button_as_link(I18n.t("formats.local_transaction.local_authority_website", local_authority_name: "Westminster"), href: "http://www.westminster.gov.uk/bear-the-cost-of-grizzly-ownership-2016-update", rel: "external")
      end
  ...
  context "given a local transaction without an interaction present" do
    ...
    context "when visiting the local transaction with a valid postcode" do
      ...
      it "links to the council website" do
        expect(page).to have_button_as_link(I18n.t("formats.local_transaction.local_authority_website", local_authority_name: "Westminster"), href: "http://westminster.example.com", rel: "external")
      end
  ...
  context "when a service is unavailable for a devolved administration with an interaction present" do
    ...
    it "shows a button that links to the local authority website" do
      expect(page).to have_button_as_link(I18n.t("formats.local_transaction.local_authority_website", local_authority_name: "Cardiff"), href: "http://cardiff.example.com", rel: "external")
    end
  end
  ...
  context "when a service is handled differently for a devolved administration" do
    ...
    it "shows a button that links to an alternate service provider" do
      expect(page).to have_button_as_link(I18n.t("formats.local_transaction.find_info_for", country_name: @country_name), href: "https://scot.gov/service", rel: "external")
    end
  end
end
