...
RSpec.describe "LocalTransactions", type: :feature do
  ...
  before do
    ...
    @payload = { analytics_identifier: nil, base_path: "/pay-bear-tax", content_id: "d6d6caaf-77db-47e1-8206-30cd4f3d0e3f", document_type: "local_transaction", first_published_at: "2016-02-29T09:24:10.000+00:00", format: "local_transaction", locale: "en", phase: "beta", public_updated_at: "2014-12-16T12:49:50.000+00:00", publishing_app: "publisher", rendering_app: "frontend", schema_name: "local_transaction", title: "Pay your bear tax", updated_at: "2017-01-30T12:30:33.483Z", withdrawn_notice: {}, links: {}, description: "Descriptive bear text.", details: { lgsl_code: 461, lgil_override: 8, service_tiers: %w[county unitary], introduction: "Information about paying local tax on owning or looking after a bear.", scotland_availability: { "type" => "devolved_administration_service", "alternative_url" => "https://scot.gov/service" }, wales_availability: { "type" => "unavailable" } }, external_related_links: [] }
    stub_content_store_has_item("/pay-bear-tax", @payload)
  end

  context "given a local transaction with an interaction present" do
    before do
      stub_local_links_manager_has_a_link(authority_slug: "westminster", lgsl: 461, lgil: 8, url: "http://www.westminster.gov.uk/bear-the-cost-of-grizzly-ownership-2016-update", country_name: "England", status: "ok", local_custodian_code: 5990)
    end
    ...
    context "when visiting the local transaction without specifying a location" do
      ...
      it "adds google analytics tags for postcodeSearchStarted" do
        ...
        expect(track_category).to eq("postcodeSearch:local_transaction")
    ...
    context "when visiting the local transaction with a valid postcode" do
      ...
      it "shows a get started button which links to the interaction" do
        assert_has_button_as_link(I18n.t("formats.local_transaction.local_authority_website", local_authority_name: "Westminster"), href: "http://www.westminster.gov.uk/bear-the-cost-of-grizzly-ownership-2016-update", rel: "external")
      end
      ...
      it "does not show a postcode error" do
        expect(page.has_selector?(".location_error")).to be false
      end

      it "does not show link to change location" do
        expect(page.has_link?("(change location)")).to be false
      end
    end
    ...
    context "when visiting the local transaction with an incorrect postcode" do
      ...
      it "populate google analytics tags" do
        track_action = page.find(".gem-c-error-summary")["data-track-action"]
        track_label = page.find(".gem-c-error-summary")["data-track-label"]

        expect(track_action).to eq("postcodeErrorShown: fullPostcodeNoLocationsApiMatch")
        expect(track_label).to eq(I18n.t("formats.local_transaction.valid_postcode_no_match"))
      end
    end
    ...
    context "when visiting the local transaction with an invalid postcode" do
      ...
      it "populates google analytics tags" do
        track_action = page.find(".gem-c-error-summary")["data-track-action"]
        track_label = page.find(".gem-c-error-summary")["data-track-label"]

        expect(track_action).to eq("postcodeErrorShown: invalidPostcodeFormat")
        expect(track_label).to eq(I18n.t("formats.local_transaction.invalid_postcode"))
      end
    end
    ...
    context "when visiting the local transaction with a valid postcode that has no local authority" do
      ...
      it "populates google analytics tags" do
        ...
        expect(track_action).to eq("postcodeErrorShown: noLaMatch")
        expect(track_label).to eq(I18n.t("formats.local_transaction.no_local_authority"))
      end
    end
  ...
  context "given a local transaction without an interaction present" do
    ...
    context "when visiting the local transaction with a valid postcode" do
      ...
      it "links to the council website" do
        assert_has_button_as_link(I18n.t("formats.local_transaction.local_authority_website", local_authority_name: "Westminster"), href: "http://westminster.example.com", rel: "external")
      end
      ...
      it "adds google analytics tags" do
        ...
        expect(track_category).to eq("userAlerts:local_transaction")
        expect(track_action).to eq("postcodeResultShown:laMatchNoLink")
      end
    end
  end
  ...
  context "given no interaction present and a missing homepage url" do
    ...
    it "adds google analytics tags" do
      ...
      expect(track_category).to eq("userAlerts:local_transaction")
      expect(track_action).to eq("postcodeResultShown:laMatchNoLinkNoAuthorityUrl")
    end
  end
  ...
  context "when a service is unavailable for a devolved administration with an interaction present" do
    before do
      ...
      stub_local_links_manager_has_a_link(authority_slug: "cardiff", lgsl: 461, lgil: 8, url: "https://www.cardiff.gov.uk/bear-the-cost-of-grizzly-ownership", country_name: "Wales", status: "ok")
      visit "/pay-bear-tax"
    ...
    it "shows a button that links to the local authority website" do
      assert_has_button_as_link(I18n.t("formats.local_transaction.local_authority_website", local_authority_name: "Cardiff"), href: "http://cardiff.example.com", rel: "external")
    end
  end
  ...
  context "when a service is handled differently for a devolved administration" do
    before do
      ...
      stub_local_links_manager_has_a_link(authority_slug: "edinburgh", lgsl: 461, lgil: 8, url: "http://www.edinburgh.gov.uk/bear-the-cost-of-grizzly-ownership", country_name: "Scotland", status: "ok")
      visit "/pay-bear-tax"
    ...
    it "shows a button that links to an alternate service provider" do
      assert_has_button_as_link(I18n.t("formats.local_transaction.find_info_for", country_name: @country_name), href: "https://scot.gov/service", rel: "external")
    end
  end
end
