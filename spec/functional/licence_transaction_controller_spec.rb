...
RSpec.describe LicenceTransactionController, type: :controller do
  ...
  context "POST to find" do
    before do
      @payload = { base_path: "/find-licences/new-licence", document_type: "licence_transaction", schema_name: "specialist_document", title: "Licence to kill", public_updated_at: "2012-10-02T12:30:33.483Z", description: "Descriptive licence text.", details: { body: "You only live twice, Mr Bond.\n", metadata: { licence_transaction_licence_identifier: "1071-5-1" } } }
    ...
    context "loading the licence edition when posting a location" do
      before do
        stub_licence_exists("1071-5-1/00BK", "isLocationSpecific" => true, "isOfferedByCounty" => false, "geographicalAvailability" => %w[England Wales], "issuingAuthorities" => [{ "authorityName" => "Staffordshire", "authoritySlug" => "staffordshire", "authorityInteractions" => { "apply" => [{ "url" => "/licence-to-kill/ministry-of-love/apply-1", "description" => "Apply for your Licence to kill", "payment" => "none", "introduction" => "", "usesLicensify" => true }] } }])
