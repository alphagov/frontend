...
def create_device_type_csv_file(data, type)
  device_categories = data.first.device_categories - %w[Totals]
...
class MonthlyBrowserData
  ...
  def browser_os_combo_delta_data
    ...
    browser_os_combo_percentage_data.map { |combo, value|
      delta = if pm_data
                value - (pm_data[combo] || 0.0)
              end
      [combo, delta]
    }.to_h
  end
  ...
  def os_session_delta_data
    ...
    os_session_percentage_data.map { |os_name, value|
      delta = if pm_data
                value - (pm_data[os_name] || 0.0)
              end
      [os_name, delta]
    }.to_h
  end
  ...
  def device_category_session_data
    device_categories.index_with { |device_category| totals[device_category] }
  end
  ...
  def device_category_delta_data
    ...
    device_category_percentage_data.map { |device_category, value|
      delta = if pm_data
                value - (pm_data[device_category] || 0.0)
              end
