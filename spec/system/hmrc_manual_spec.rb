RSpec.describe "Hmrc manual" do
  context "when visiting a HMRC manual page" do
    let(:content_item) { GovukSchemas::Example.find(:hmrc_manual, example_name: "hmrc_manual") }
    ...
    it "displays the title" do
      visit base_path
      expect(page).to have_title("VAT Government and Public Bodies")
    end

    it "displays the description" do
      visit base_path
      expect(page).to have_text("Guidance on VAT as it applies to local authorities and other government and public bodies.")
    end

    it "displays the manual type" do
      visit base_path
      within "#manual-title" do
        expect(page).to have_text("HMRC internal manual")
      end
    end
  end
