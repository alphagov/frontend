RSpec.describe "ContentThenSidebarLayout flexible section" do
  subject(:flexible_section) { FlexiblePage::FlexibleSection::SidebarThenContentLayout.new(content:, sidebar:) }
  ...
  before do
    render(template: "flexible_page/flexible_sections/_sidebar_then_content_layout", locals: { flexible_section: })
  end

  it "renders 1 column then 2 columns" do
    expect(rendered).to have_selector(".govuk-grid-column-one-third + .govuk-grid-column-two-thirds")
  end
