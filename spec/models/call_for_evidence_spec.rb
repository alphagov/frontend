RSpec.describe CallForEvidence do
  ...
  # it_behaves_like "it can have metadata", "call_for_evidence", "open_call_for_evidence"

  it_behaves_like "it can have national applicability", "call_for_evidence", "open_call_for_evidence"

  it_behaves_like "it can have historical government information", "call_for_evidence", "open_call_for_evidence"

  it_behaves_like "it can have shareable links", "call_for_evidence", "open_call_for_evidence"
  ...
  describe "#outcome_detail" do
    it "returns information if it has the call_for_evidence_outcome document type" do
      expect(call_for_evidence_outcome.outcome_detail).to be(call_for_evidence_outcome.content_store_response["details"]["outcome_detail"])
    end
  ...
  describe "#general_documents" do
    it "returns featured attachments if available for any call for evidence document type" do
      expect(call_for_evidence_outcome_with_featured_attachments.general_documents.length).to be(1)

      expect(unopened_call_for_evidence.general_documents).to be_empty
      expect(open_call_for_evidence.general_documents).to be_empty
      expect(closed_call_for_evidence.general_documents).to be_empty
    end
  end

  describe "#held_on_another_website_url" do
    it "returns url if it is held on another website" do
      expect(open_call_for_evidence.held_on_another_website_url).to be(open_call_for_evidence.content_store_response["details"]["held_on_another_website_url"])
    end
    ...
    it "does not return url if it is not held on another website" do
      ...
      expect(closed_call_for_evidence.held_on_another_website_url).to be_nil
      expect(call_for_evidence_outcome.held_on_another_website_url).to be_nil
    end
  end
  ...
  describe "#held_on_another_website?" do
    ...
    it "returns false if it is not held on another website" do
      ...
      expect(closed_call_for_evidence.held_on_another_website?).to be(false)
      expect(call_for_evidence_outcome.held_on_another_website?).to be(false)
    end
  end
  ...
  describe "#ways_to_respond?" do
    it "returns true for an open_call_for_evidence document type that has contact information available" do
      expect(open_call_for_evidence_with_participation.ways_to_respond?).to be_present
      expect(open_call_for_evidence_with_participation.email).to be_present
      expect(open_call_for_evidence_with_participation.respond_online_url).to be_present
      expect(open_call_for_evidence_with_participation.postal_address).to be_present
    end

    it "returns false for an open_call_for_evidence document type that only has an attachment url as contact information" do
      ways_to_respond = open_call_for_evidence_with_participation.content_store_response["details"]["ways_to_respond"]
      ways_to_respond.delete("email")
      ways_to_respond.delete("postal_address")
      ways_to_respond.delete("link_url")

      expect(open_call_for_evidence_with_participation.ways_to_respond?).not_to be_present
    end
  ...
  describe "#response_form?" do
    it "returns true for an open_call_for_evidence document type that has attachment url and one of either email or postal address" do
      expect(open_call_for_evidence_with_participation.response_form?).to be_present
    ...
    it "returns false for an open_call_for_evidence document type that has attachment url but no email or postal address" do
      ...
      expect(open_call_for_evidence_with_participation.response_form?).not_to be_present
    end
  end
end
