RSpec.describe RandomController, type: :controller do
  context "random page route" do
    context "with valid search results" do
      before do
        results = { "results" => [{ "link" => "/bereavement-allowance" }, { "link" => "/book-life-in-uk-test" }, { "link" => "http://www.wyreforestdc.gov.uk" }] }
    ...
    context "with invalid search results" do
      before do
        results = { "results" => [{ "link" => "http://www.wyreforestdc.gov.uk" }, { "link" => "/bereavement-allowance" }, { "link" => "/book-life-in-uk-test" }] }
        stub_request(:get, "#{Plek.new.find('search-api')}/search.json?count=0").to_return(status: 200, body: "{ \"total\": 3 }")
        stub_request(:get, /#{Plek.new.find('search-api')}\/search.json\?count=1&fields=link&start=./).to_return(status: 200, body: results.to_json)
      end
      ...
      it "does not redirect to URLs that start with 'http' and start again" do
        get(:random_page)
        assert_response(:redirect)
        expect(response.redirect_url).to eq("#{Plek.new.website_root}/random")
      end
